...
### Code
```C
int is_number(char c)	#include <unistd.h>
{	int	exponent(int base, int pwr)
	if (c >= '0' && c <= '9')	
		return (1);	
	return (0);	
...
int	ft_atoi(char *str)	void	ft_putchar(char c)
{	{
	int	i;		write(1, &c, 1);
	int sign;	}
	int nums_left;	
	int out;	
	i = -1;	int	edge_cases(int nb)
	sign = 1;	{
	out = 0;		if (nb == 0 || nb == -2147483648)
	...
	i = 0;	
	while (--nums_left>=0 && is_number(str[i]))	void	ft_putnbr(int nb)
{
	int	pwr;
	int	dgt;
	int	write_bool;
	...
	if (nb < 0)
	{		{
		out += exponent(10, nums_left) * (str[i] - '0');			ft_putchar('-');
		i++;			nb *= -1;
	}
	while (--pwr >= 0)
	{
