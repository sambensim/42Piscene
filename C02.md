...
```
### ex10 - ft_strlcpy BUZZ BUZZ
##### Problem NONONONO
Reproduce the behavior of the function strlcpy (man strlcpy).
> This is the same as ex01, the only differences being that we return the number of chars copied, only end with one null character, and size includes a byte for the null character
___
##### Code UHOH
```C
